...
public class TST<Value> {
    ...
    public Iterable<String> keysWithPrefix(String prefix) {
        ...
        if (x.val != null) queue.offerLast(prefix);
        collect(x.mid, new StringBuilder(prefix), queue);
        return queue;
    }

    // all keys in subtrie rooted at x with given prefix
    private void collect(Node<Value> x, StringBuilder prefix, Deque<String> queue) {
        if (x == null) return;
        collect(x.left,  prefix, queue);
        if (x.val != null) queue.offerLast(prefix.toString() + x.c);
    ...
    private void collect(Node<Value> x, StringBuilder prefix, int i, String pattern, Deque<String> queue) {
        if (x == null) return;
        char c = pattern.charAt(i);
        if (c == '.' || c < x.c) collect(x.left, prefix, i, pattern, queue);
        if (c == '.' || c == x.c) {
            if (i == pattern.length() - 1 && x.val != null) queue.offerLast(prefix.toString() + x.c);
